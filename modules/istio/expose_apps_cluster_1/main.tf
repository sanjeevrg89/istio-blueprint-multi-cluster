...
#Expose ISTIOD Cluster1
resource "kubernetes_manifest" "istiod_gateway1" {
...
resource "kubernetes_manifest" "istiod_virtual_service1" {
  provider = kubernetes.cluster1
  manifest = {
    apiVersion = "networking.istio.io/v1alpha3"
    kind       = "VirtualService"
...
resource "kubernetes_secret" "istio_remote_secret_cluster1" {
  ...
  metadata {
    ...
    name      = "istio-remote-secret-${data.terraform_remote_state.eks_cluster_1.outputs.cluster1_name}"
    namespace = "istio-system"
  }
  ...
  data = {
    cluster1_name = templatefile("${path.module}/istio-remote-secret.tftpl",
      {
        cluster_certificate_authority_data = data.terraform_remote_state.eks_cluster_1.outputs.cluster1_certificate_authority_data
        cluster_host                       = data.terraform_remote_state.eks_cluster_1.outputs.cluster1_endpoint
        cluster_name                       = data.terraform_remote_state.eks_cluster_1.outputs.cluster1_name
